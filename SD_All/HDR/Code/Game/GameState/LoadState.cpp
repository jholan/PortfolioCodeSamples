...
void LoadState::Initialize(Game* owner)
{
	...
	// Background
	const Texture* loadScreen = Texture::CreateOrGet(g_gameConfigBlackboard.GetValue("loadStateBackground", "Data/BuiltIns/DEFAULT_TEXTURE.png"), Texture::TEXTURE_FORMAT_RGBA8);
...
void LoadState::LoadTextures()
{
	...
	if (!textureFilepaths.empty())
	{
		...
		for (int i = 0; i < (int)splitFilepaths.size(); ++i)
		{
			...
			Texture::CreateOrGet(textureFilepath, Texture::TEXTURE_FORMAT_RGBA8);
		}
	}
}
