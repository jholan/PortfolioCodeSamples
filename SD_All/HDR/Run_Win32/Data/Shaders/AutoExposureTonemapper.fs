...
void main( void )
{
	...
	//else
	//{
		if (gl_FragCoord.y >= 400 && gl_FragCoord.y <= 500 &&
			gl_FragCoord.x >= 200 && gl_FragCoord.x <= 400)
		{
			float l = textureLod(gTexCurrLuminanceMap, passUV, MIP_LEVEL_LCT).x;
			outColor = vec4(l, l, l, 1.0f);
		}
		
		if (gl_FragCoord.y >= 525 && gl_FragCoord.y <= 625 &&
			gl_FragCoord.x >= 200 && gl_FragCoord.x <= 400)
		{
			float l = textureLod(gTexLuminanceMap, passUV, MIP_LEVEL).x;
			outColor = vec4(l, l, l, 1.0f);
		}
