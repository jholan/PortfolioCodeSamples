...
void DevConsole::Initialize()
{
	...
	m_positionIndicator = Texture::CreateOrGet("Data/Fonts/TextPositionIndicator.png", Texture::TEXTURE_FORMAT_RGBA8);
...
void DevConsole::BeginFrame(float deltaSeconds)
{
	...
	LogEntry le;
	while(m_threadedLogQueue.Dequeue(&le))
	{
		PrintToLog(le.m_drawColor, le.m_text);
	}
}
...
void DevConsole::AddThreadedEntry(const std::string& entry)
{
	LogEntry le;
	le.m_drawColor = RGBA();
	le.m_text = entry;

	m_threadedLogQueue.Enqueue(le);
}
