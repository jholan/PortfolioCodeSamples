...
void Light::TryAllocateDefaultShadowMap()
{
	...
	{
		// If we have no defualt shadow map, create one
		s_defaultShadowMap = g_theRenderer->CreateDepthStencilTarget(shadowMapResolution, shadowMapResolution);
		Camera camera;
...
Texture* Light::CreateOrGetDepthTargetForUpdating(Texture** testDepthTarget)
{
	...
	if (IsShadowCasting())
	{
		if (*testDepthTarget == nullptr)
		{
			...
			int shadowMapResolution = GetShadowMapResolution();
			*testDepthTarget = g_theRenderer->CreateDepthStencilTarget(shadowMapResolution, shadowMapResolution);
