...
Sprite::Sprite(const XMLElement& definition, bool flippedY)
{
	...
	if (diffuseTextureElement)
	{
		...
		m_texture = Texture::CreateOrGet(textureFilepath, Texture::TEXTURE_FORMAT_RGBA8);
...
Sprite* Sprite::Get( const std::string& name )
{
	...
	if (alreadyExists == s_loadedSprites.end())
	{
		...
		sprite = new Sprite(Texture::CreateOrGet(BAD_FILEPATH, Texture::TEXTURE_FORMAT_RGBA8), AABB2D(Vector2(0.0f, 0.0f), Vector2(1.0f, 1.0f)), Vector2(0.5f, 0.5f), Vector2(1.0f, 1.0f));
