...
void MaterialData::TryInitializeStatics()
{
	...
	if (!s_isInitialized)
	{
		...
		DEFUALT_DIFFUSE_TEXTURE = Texture::CreateOrGet("Data/BuiltIns/DEFAULT_TEXTURE.png");
		DEFAULT_NORMAL_MAP = Texture::CreateOrGet("Data/BuiltIns/NORMAL_MAP_FLAT.png");
...
MaterialData::MaterialData(const XMLElement& definition)
{
	...
	while(textureElement != nullptr)
	{
		...
		SetTexture(bindPoint, filepath, samplerType);
	...
	if (diffuseElement != nullptr)
	{
		...
		SetDiffuseTexture(filepath, samplerType);
	}
	...
	if (normalMapElement != nullptr)
	{
		...
		SetNormalMap(filepath, samplerType);
	}
...
void MaterialData::SetTexture(unsigned int bindPoint, const std::string& filepath, TextureSamplerType samplerType)
{
	...
	SetTexture( bindPoint, Texture::CreateOrGet(filepath, generateMipmaps), samplerType);
}
...
void MaterialData::SetDiffuseTexture(const std::string& filepath, TextureSamplerType samplerType)
{
	// If we are using a mip map sampler we want to generate mip maps for our texture
	bool generateMipmaps = false;
	if (samplerType == TEXTURE_SAMPLER_TYPE_LINEAR_MIPMAPS || samplerType == TEXTURE_SAMPLER_TYPE_POINT_MIPMAPS)
	...
	if (!filepath.empty())
	{
		SetDiffuseTexture(Texture::CreateOrGet(filepath, generateMipmaps), samplerType);
	}
	else
	{
...
void MaterialData::SetNormalMap(const std::string& filepath, TextureSamplerType samplerType)
{
	// If we are using a mip map sampler we want to generate mip maps for our texture
	bool generateMipmaps = false;
	if (samplerType == TEXTURE_SAMPLER_TYPE_LINEAR_MIPMAPS || samplerType == TEXTURE_SAMPLER_TYPE_POINT_MIPMAPS)
	...
	if (!filepath.empty())
	{
		SetNormalMap(Texture::CreateOrGet(filepath, generateMipmaps), samplerType);
	}
	else
	{
...
void MaterialData::ParseMaterialDatas(const std::string& materialFile)
{
	if (DEFAULT_MATERIAL == nullptr)
	{
		...
		DEFAULT_MATERIAL->SetDiffuseTexture("Data/BuiltIns/BAD_MATERIAL.png");
	}
...
void Material::SetTexture(unsigned int bindPoint, const std::string& filepath, TextureSamplerType samplerType)
{
	TryAllocateMutableMaterial();
	GetActiveMaterial()->SetTexture(bindPoint, filepath, samplerType);
}
...
void Material::SetDiffuseTexture(const std::string& filepath, TextureSamplerType samplerType)
{
	TryAllocateMutableMaterial();
	GetActiveMaterial()->SetDiffuseTexture(filepath, samplerType);
}
...
void Material::SetNormalMap(const std::string& filepath, TextureSamplerType samplerType)
{
	TryAllocateMutableMaterial();
	GetActiveMaterial()->SetNormalMap(filepath, samplerType);
}
